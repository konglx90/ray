...
from ws.client import ws_start, ws
...
def parse_question_and_answer(text_list):
    ...
    if (start == 0):
        start = -3
        for i, keyword in enumerate(text_list[0: -3]):
            question += keyword
...
def main():
    ...
    def __inner_job():
        ...
        # send message to websocket, finally show in broswer
        ws.send(json.dumps({'question': real_question, 'options': answers}))
    ...
    # start websocket to show beautiful look in broswer
    ws_sync_job = threading.Thread(target=ws_start)
    ws_sync_job.daemon = True
    ws_sync_job.start()
